...
public class MultipleInventoryFactory {
    ...
    public MultipleInventoryFactory(List<ItemStack> items, InventoryFactory startFactory) {
        ...
        for (int i = 0; i < totalPages; i++) {
            ...
            startFactory.fill(new ItemStackFactory(Material.BLACK_STAINED_GLASS_PANE)
                    .setName(" ").get());

            for (int slot : airSlots) {
                startFactory.setItem(slot, new ItemStack(Material.AIR));
            }

            startFactory.addItem(itemsToDisplayInThisPage);
            setPageNavigationItems(i, totalPages, startFactory);

            pages.add(startFactory.get());
        }
    }
    ...
    private void setPageNavigationItems(int pageNumber, int totalPages, InventoryFactory template) {
        ...
        if (pageNumber > 0) {
            ...
            template.setItem(27, new ItemStackFactory(Material.ARROW)
                    .addLoreLine(StringUtils.formatColorCodes('&', "&cPagina precedente"))
        ...
        if (pageNumber < totalPages - 1) {
            ...
            template.setItem(36, new ItemStackFactory(Material.SPECTRAL_ARROW)
                            .addLoreLine(StringUtils.formatColorCodes('&', "&aPagina successiva"))
